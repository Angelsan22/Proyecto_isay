                ...
                @foreach($autopartes as $ap)
                    <div class="col">
                        <div class="card text-center border shadow-sm h-100 p-2">
                            <img src="{{ $ap->imagen_url }}" style="height:80px; object-fit:contain;" class="mb-1" alt="{{ $ap->nombre }}">
                            <p class="small fw-semibold mb-0" style="font-size:.75rem;">{{ $ap->nombre }}</p>
                            <p class="small text-muted mb-1">${{ number_format($ap->precio, 2) }}</p>
